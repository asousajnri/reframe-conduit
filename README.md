...
* [`src/reframe_conduit/`](src/reframe_conduit/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/reframe_conduit/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `reframe-conduit` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/reframe_conduit/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns reframe-conduit.example
  (:require [reframe-conduit.config :as config])
